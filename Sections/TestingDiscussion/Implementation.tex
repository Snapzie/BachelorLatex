...
Semantically, we have split these combinators into three parts: \textit{'boss}, which contains the tests that are only dependent on the boss being a part of the game. That is, these tests do not use the player's abilities, but only the boss. The next semantic type is \textit{'bossPresent}. These combinators include tests that rely both on the player's ability and the boss. However, since we only have a single case where these overlap, it is only the \textit{'bossPresent('fireball, 'variationWithBoss)} that actually inserts a test, as there are no tests of roar and boss. The last semantic type is \textit{'testAbility} which contains the tests that are only specific for the player's ability. With these three parts, we can succesfully combinate all the possible variations of player tests that vary depending on ability and boss presence. \todo{nævn ingen roar + boss tests? og none}
\\ 
With this in mind, we can then describe player tests as: \textit{string $\cap$ boss, string $\cap$ bossPresent, string $\cap$ testAbility $\to$ MyResult $\cap$ playerTest}\todo{forklar bedre? Er det korrekt?}
%Player Integration

%Semantically, we have a \textit{'testAbility} that depends on \textit{'bossPresent} that specifies our boss' existence, along with the player's ability. This way we can describe the player test as: \textit{string $\cap$ boss, string $\cap$ bossPresent, string $\cap$ testAbility $\to$ MyResult $\cap$ playerTest}\todo{forklar bedre?}. 

%The player tests are the only unit tests that have this form of overlap with variations. For instance, the unit tests of boss is simpler. The function that determines the semantic types look for which BossAbilityType is present, where the combinators are of type \textit{'bossTest} with a given BossAbilityType. Boss can then simply be described as: \textit{string $\cap$ 'bossTest $\to$ string $\cap$ 'bossTestAbility}. Room unit tests are largely similiar and in the same simple form as the boss unit tests. \\
%However, we do also have the case where the boss is not present and should therefore not have tests for boss at all. We have a special combinator of type \textit{'bossTestAbility('none)} that generates an empty file for boss tests. This non-boss can be described as: \textit{string $\cap$ 'bossTestAbility} as it takes no \textit{'bossTest} as it has no abilities. \todo{Er alt dette trash? Ja.}
%Semantics, bossPresent(yadda)
%Different implementation?
%Other unit tests are the same
\subsubsection{Boss}
